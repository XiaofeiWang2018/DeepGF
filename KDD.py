...
summary_save_dir= "summaryKDD/"
acc_txt="accKDD.txt"
log_txt="logKDD.txt"
test_details="test_detailKDD.txt"
config_txt="configKDD.txt"
throw_txt="throwKDD.txt"
save_path='modelKDD'
with open("configKDD.txt", "w+")as f0:
    f0.write('mode: ' + mode)
    f0.write('\n')
    f0.flush()
...
def main():
    ...
    label_predict_op = net.inference_model_KDD(input,label_attention_map,label_polar_map,delta_year,init_a,init_b)  # label_predict_op=(batch, n_steps, n_outputs)
                        ...
                        if epoch % epoch_test == 0:
                            ...
                            for j in range(int(test_num / batch_size_val)):
                                ...
                                for nb in range(batch_size_val*n_steps): #20
                                    ...
                                    y_scores[j*(batch_size_val*n_steps)+nb]=(math.exp(label_predict_1[nb]))/(math.exp(label_predict_1[nb])+math.exp(label_predict_0[nb]))
                                ...
                                for batch in range(batch_size_val):
                                    ...
                                    f6.flush()
                                    for img_perimgpath in range(5):
                                        p_glau=math.exp(label_predict[batch][img_perimgpath][1] )/( math.exp(label_predict[batch][img_perimgpath][1]) + math.exp(label_predict[batch][img_perimgpath][0]) )
                            ...
                            f.write(
                                ' epoch %03d  | Acc: %.3f%% | sen: %.3f%% | spe: %.3f%%  |AUC: %.3f%%|tp: %.3f | tn: %.3f| fp: %.3f | fn: %.3f' % (
                                    epoch, 100. * acc, 100. * Sen, 100. * Spe,100. * AUC,tp, tn, fp, fn))
                            f.write('\n')
                            f.flush()
                            dataloader_test = DataLoader_atten_polar(batch_size=batch_size,list_img_path=list_img_path_test, state='test')

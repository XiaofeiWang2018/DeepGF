...
class Net(BasicNet_3.BasicNet):
    ...
    def get_label_attention_polar_inference1(self, x, label_attention_map_norm,polar,reuse):
        label_attention_map_norm = label_attention_map_norm * (1 - 0.2) + 0.2
        with tf.variable_scope('label_attention'):
            atten_out = self.predict_inference1(x, label_attention_map_norm, label_attention_map_norm,
                                               reuse=reuse)  # [:,:,:,0:1]
        with tf.variable_scope('polar'):
            polar_out = self.polar_inference1(polar, reuse=reuse)  # [:,:,:,0:1]

        cn_out = tf.concat([atten_out, polar_out], axis=3)
        ave_pool_1 = tf.reduce_mean(cn_out, reduction_indices=[1, 2], name="avg_pool")  # feature size at 1*1*512
        ...
        self.predict_label_f = fc_3
        return cn_out
    def get_label_attention_polar_inference2(self, x, label_attention_map_norm,polar,reuse):
        label_attention_map_norm = label_attention_map_norm * (1 - 0.2) + 0.2

        with tf.variable_scope('polar'):
            polar_out = self.polar_inference2(polar, reuse=reuse)  # [:,:,:,0:1]
        ...
        self.predict_label_f = fc_3
        return cn_out
    def get_label_attention_polar_inference3(self, x, label_attention_map_norm,polar,reuse):
        label_attention_map_norm = label_attention_map_norm * (1 - 0.2) + 0.2
        with tf.variable_scope('label_attention'):
            atten_out = self.predict_inference3(x, label_attention_map_norm, label_attention_map_norm,
                                               reuse=reuse)  # [:,:,:,0:1]
    ...
    def get_label_only_polar_inference(self, x, label_attention_map_norm,polar,reuse):
        label_attention_map_norm = label_attention_map_norm * (1 - 0.2) + 0.2
        with tf.variable_scope('label_attention'):
            atten_out = self.predict_inference(x, label_attention_map_norm, label_attention_map_norm,
                                               reuse=reuse)  # [:,:,:,0:1]
    ...
    def predict_inference1(self, x, salmask_input, salmask_feature,
                          reuse=False):  # x size at 224*224*3,(-1,1) , salmask(0,1)

        x = self.conv_mask(x, salmask_input)

        self.mask_input = x
        conv_1 = self.conv('1conv_1', x, ksize=7, filters_out=80, stride=2, reuse=reuse)
        conv_1 = self.conv_mask(conv_1, salmask_feature)

        # feature size at 56*56*64
        max_pool_1 = self.max_pool(conv_1, ksize=3, stride=2)
        block_1_1 = self.vv_building_block('1block_1_1', max_pool_1, output_channel=80, stride=1, reuse=reuse)
        block_1_1 = self.conv_mask(block_1_1, salmask_feature)

        # feature size at  28*28*128
        block_2_1 = self.vv_building_block('1block_2_1', block_1_1, output_channel=160, stride=2, reuse=reuse)
        block_2_1 = self.conv_mask(block_2_1, salmask_feature)

        # # feature size at 14*14*128
        block_3_1 = self.vv_building_block('1block_3_1', block_2_1, output_channel=160, stride=2, reuse=reuse)
        block_3_1 = self.conv_mask(block_3_1, salmask_feature)
    ...
    def polar_inference1(self, x, reuse=False):
        conv_1 = self.conv('1conv_1', x, ksize=7, filters_out=40, stride=2, reuse=reuse)

        # feature size at 56*56*64
        max_pool_1 = self.max_pool(conv_1, ksize=3, stride=2)
        block_1_1 = self.vv_building_block('1block_1_1', max_pool_1, output_channel=40, stride=1, reuse=reuse)

        # feature size at  28*28*128
        block_2_1 = self.vv_building_block('1block_2_1', block_1_1, output_channel=40, stride=2, reuse=reuse)

        # # feature size at 14*14*128
        block_3_1 = self.vv_building_block('1block_3_1', block_2_1, output_channel=80, stride=2, reuse=reuse)
    ...
    def polar_inference2(self, x, reuse=False):
        conv_1 = self.conv('1conv_1', x, ksize=7, filters_out=84, stride=2, reuse=reuse)

        # feature size at 56*56*64
        max_pool_1 = self.max_pool(conv_1, ksize=3, stride=2)
        block_1_1 = self.vv_building_block('1block_1_1', max_pool_1, output_channel=84, stride=1, reuse=reuse)

        # feature size at  28*28*128
        block_2_1 = self.vv_building_block('1block_2_1', block_1_1, output_channel=84, stride=2, reuse=reuse)

        # # feature size at 14*14*128
        block_3_1 = self.vv_building_block('1block_3_1', block_2_1, output_channel=168, stride=2, reuse=reuse)

        # feature size at 7*7*256
        block_4_1 = self.vv_building_block('1block_4_1', block_3_1, output_channel=320, stride=2, reuse=reuse)

        return block_4_1  # ave_pool_1

    def predict_inference3(self, x, salmask_input, salmask_feature,
                          reuse=False):  # x size at 224*224*3,(-1,1) , salmask(0,1)

        x = self.conv_mask(x, salmask_input)

        self.mask_input = x
        conv_1 = self.conv('1conv_1', x, ksize=7, filters_out=69, stride=2, reuse=reuse)
        conv_1 = self.conv_mask(conv_1, salmask_feature)

        # feature size at 56*56*64
        max_pool_1 = self.max_pool(conv_1, ksize=3, stride=2)
        block_1_1 = self.vv_building_block('1block_1_1', max_pool_1, output_channel=69, stride=1, reuse=reuse)
        block_1_1 = self.conv_mask(block_1_1, salmask_feature)

        # feature size at  28*28*128
        block_2_1 = self.vv_building_block('1block_2_1', block_1_1, output_channel=138, stride=2, reuse=reuse)
        block_2_1 = self.conv_mask(block_2_1, salmask_feature)

        # # feature size at 14*14*128
        block_3_1 = self.vv_building_block('1block_3_1', block_2_1, output_channel=138, stride=2, reuse=reuse)
        block_3_1 = self.conv_mask(block_3_1, salmask_feature)

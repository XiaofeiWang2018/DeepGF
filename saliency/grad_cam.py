...
class GradCam(SaliencyMask):
    ...
    def __init__(self, graph, session, y, x, conv_layer):
        ...
        self.gradients_node=[]
        for j in range(5):
            self.gradients_node.append(tf.gradients(y[j], conv_layer)[0])
    ...
    def GetMask(self, x_value, feed_dict={}, should_resize = True, three_dims = True):
        ...
        grad_cams=[]
        (output0, grad0) = self.session.run([self.conv_layer, self.gradients_node],
                                          feed_dict=feed_dict)
        for img_index in range(5):
            # conv_layer_PerImg=self.conv_layer[:,img_index,:,:,:]
            # y_PerImg=self.y[img_index]
            # gradients_node=tf.gradients(y_PerImg, conv_layer_PerImg)[0]
            # (output, grad) = self.session.run([self.conv_layer[:,img_index,:,:,:], self.gradients_node[img_index]],feed_dict=feed_dict)

            output = output0[:, img_index, :, :, :][0]
            grad = grad0[4][0][4]
            ...
            if should_resize:
                ...
                with self.graph.as_default():
                    grad_cam = np.squeeze(tf.image.resize_bilinear(np.expand_dims(np.expand_dims(grad_cam, 0), 3),x_value.shape[1:3]).eval(session=self.session))

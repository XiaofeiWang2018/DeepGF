...
class Net(BasicNet.BasicNet):
    ...
    def inference_model_1(self, x,label_attention_map,label_polar_map,delta_year,init_a,init_b,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       CVPR(polar)+Lstm(no timegate)+GAP+fc
        """

        cnn_sequential = self.multi_CNNoutput_pretrainedCVPR_polar1(x,label_attention_map,label_polar_map,is_training=is_training)

        lstm_input = tf.reduce_mean(cnn_sequential,axis=(2,3)) #[batch_size,n_steps,256+64]

        stacked_outputs_0 = tf.layers.dense(lstm_input, 256, name='fc0')
        ...
        return stacked_outputs_1



    def inference_model_2(self, x,label_attention_map,label_polar_map,delta_year,init_a,init_b,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       CVPR(polar)+Lstm(no timegate)+GAP+fc
        """

        cnn_sequential = self.multi_CNNoutput_pretrainedCVPR_polar2(x, label_attention_map, label_polar_map,
    ...
    def inference_model_3(self, x,label_attention_map,label_polar_map,delta_year,init_a,init_b,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       CVPR(polar)+Lstm(no timegate)+GAP+fc
        """

        cnn_sequential = self.multi_CNNoutput_pretrainedCVPR_polar3(x, label_attention_map, label_polar_map,
                                                                    is_training=is_training)
    ...
    def inference_model_KDD(self, x,label_attention_map,label_polar_map,delta_year,init_a,init_b,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       CVPR(polar)+Lstm(no timegate)+GAP+fc
        """
        ...
        fc_output = self.lstm_gap_fc_timegateKDD('lstm8', lstm_input, n_layers=self.n_layers,
                                         is_training=is_training,delta_year=delta_year,init_a=init_a,init_b=init_b)  # [batch, n_steps, 28,28,64]
    ...
    def inference_model_9(self, x,label_attention_map,label_polar_map,delta_year,init_a,init_b,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       rebuttal visualization
        ...
        self.predict_label = fc_output
        return fc_output ,cnn_sequential
    ...
    def multi_CNNoutput_pretrainedCVPR_polar1(self, x, label_attention_map,polar,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps =x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            polar_0=polar[:, i, :, :, :]
            if i == 0:
                y_0=net.get_label_attention_polar_inference1(x_0, label_attention_map_0,polar_0,reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                y_0=net.get_label_attention_polar_inference1(x_0, label_attention_map_0,polar_0,reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y,y_0], 1)

        return y

    def multi_CNNoutput_pretrainedCVPR_polar2(self, x, label_attention_map,polar,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps =x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            polar_0=polar[:, i, :, :, :]
            if i == 0:
                y_0=net.get_label_attention_polar_inference2(x_0, label_attention_map_0,polar_0,reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                y_0=net.get_label_attention_polar_inference2(x_0, label_attention_map_0,polar_0,reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y,y_0], 1)

        return y

    def multi_CNNoutput_pretrainedCVPR_polar3(self, x, label_attention_map,polar,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps =x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            polar_0=polar[:, i, :, :, :]
            if i == 0:
                y_0=net.get_label_attention_polar_inference3(x_0, label_attention_map_0,polar_0,reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                y_0=net.get_label_attention_polar_inference3(x_0, label_attention_map_0,polar_0,reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y,y_0], 1)

        return y
    def multi_CNNoutput_only_polar(self, x, label_attention_map,polar,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps =x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            polar_0=polar[:, i, :, :, :]
            if i == 0:
                y_0=net.get_label_only_polar_inference(x_0, label_attention_map_0,polar_0,reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                y_0=net.get_label_only_polar_inference(x_0, label_attention_map_0,polar_0,reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y,y_0], 1)

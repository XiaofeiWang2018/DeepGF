...
class DataLoader_atten_polar(object):
    ...
    def __init__(self, batch_size, list_img_path,state='train'):
        ...
        self.path_to_polar = 'data/final_polar/'
    ...
    def get_batch_single(self):  # Returns
        if self.cursor + self.batch_size > self.size:
            self.cursor = 0
            self.batch_order = 0
            # np.random.shuffle(self.list_img_path)
        ...
        for idx in range(self.batch_size):
            ...
            image_sublist.sort()
            for idx_image in range(5):
                image=image_sublist[idx_image+1]
                img_name = os.path.split(image)[-1]
...
if __name__ == '__main__':
    ...
    txt_dir='data/train/label/all'
    list_log=os.listdir(txt_dir)
    trainlist=[]
    for i in range(1308):
        count=0
        with open(txt_dir +'/'+ list_log[i] , 'r') as f:
            K = f.readlines()
            for i_line in range(5):
                line = K[i_line + 1]
                line = line.strip('\n')
                ...
                count=count+line
        if  count>0:
            for i_count in range(20):
                trainlist.append(list_log[i][0:-4])
        else:
            trainlist.append(list_log[i][0:-4])
        print(i)

    # file = open('trainlist_wo_allzeros.txt', 'w+')
    # for fp in trainlist:
    #     file.write(str(fp))
    #     file.write('\n')
    # file.close()

...
mode='CVPR cls strategy'
test_num = 348
test_num_pre=384
lr_change= [1*1e-7, 4*1e-7, 4*1e-6, 4*1e-6, 4*1e-5, 2*1e-4]
throw_rate=0.5
trainingset_num=[1308,652,324,160,100,100]
num_strategy=5

strategy_epoch_duration=[2,2,3,3,3,3]
Epoch = 500
epoch_test = 4
epoch_save=20
init_a=100
...
summary_save_dir= "summary11/"
acc_txt="accs11.txt"
log_txt="logs11.txt"
test_details="test_details11.txt"
test_details_pre="test_detail_pres11.txt"
config_txt="configs11.txt"
throw_txt="throws11.txt"
save_path='models11'
with open("configs11.txt", "w+")as f0:
    f0.write('mode: ' + mode)
    f0.write('\n')
    f0.flush()
...
def main():
    ...
    label_predict_op ,cnn_out= net.inference_model_10(input,label_attention_map)  # label_predict_op=(batch, n_steps, n_outputs)
                ...
                for epoch in range(0, Epoch): #3*10
                    ...
                    if epoch>0 and  (epoch % strategy_epoch_duration[count_strategy]) == 0 and flag_strategy:
                        ...
                        for jj in range(int(train_leftnum_before / batch_size)):
                            image_train_test, year_train_test, GTmap_train_test, Polar_train_test, GTlabel_train_test = dataloader_strategy.get_batch_single()
                    ...
                    for i in range(int(trainingset_num[count_strategy]/batch_size)):
                        image1,year1,GTmap1,Polar1, GTlabel1= dataloader_train.get_batch_single()
                        ...
                        if count % 20 == 0:  # tensorboard
                            image2, year2, GTmap2, Polar2, GTlabel2=dataloader_tensorboard.get_batch_single()
                        ...
                        with open(test_details, "a")as f6:
                            ...
                            if epoch % epoch_test == 0:
                                ...
                                for j in range(int(test_num / batch_size_val)): #348
                                    imagev, yearv, GTmapv, Polarv, GTlabelv = dataloader_test.get_batch_single()
                                ...
                                for j in range(int(test_num_pre / batch_size_val)):
                                    imagev, yearv, GTmapv, Polarv, GTlabelv = dataloader_test_pre.get_batch_single()
                                    label_predict = sess.run(label_predict_op, feed_dict={input: imagev, GT_label: GTlabelv,
                                                                                          label_attention_map: GTmapv,
                                                                                          label_polar_map: Polarv,
                                                                                          delta_year: yearv,

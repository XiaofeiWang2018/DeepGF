    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.Session(config=tf_config, graph=graph) as sess:
            ...
            for i in range(5):
                ...
                """----------------------01------------------------------------"""
                jump = map_01[i]
                Image.imsave('visualization_result/cls_cvpr/gradcam/01/img' + str(i + 1) + '_jump_cls.jpg', mask[jump-2],
                             cmap=plt.get_cmap('gray'))
                Image.imsave(
                    'visualization_result/cls_cvpr/gradcam/01/img' + str(i + 1) + '.jpg',image[0][jump-2])
                print( str(i + 1) )
                """----------------------11------------------------------------"""
                # jump = map_11[i]
                # Image.imsave('visualization_result/cls_cvpr/gradcam/11/img' + str(i + 1) + '_glau_cls.jpg', mask[jump-2],
                #              cmap=plt.get_cmap('gray'))
                # Image.imsave(
                #     'visualization_result/cls_cvpr/gradcam/11/img' + str(i + 1) + '.jpg',image[0][jump-2])
                print( str(i + 1) )

...
class Net(BasicNet.BasicNet):
    ...
    def inference_model_9(self, x,label_attention_map,label_polar_map,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       our single :pretrained cvpr_polar
        """

        predict_label = self.single_CNNoutput_pretrainedCVPR_polar(x,label_attention_map,label_polar_map,is_training=is_training) #B,6,2
        self.predict_label = predict_label
        return predict_label

    def inference_model_10(self, x,label_attention_map,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       our single :pretrained cvpr
        """

        predict_label ,cnn_out= self.single_CNNoutput_pretrainedCVPR(x,label_attention_map,is_training=is_training) #B,6,2
        self.predict_label = predict_label
        return predict_label,cnn_out

    def inference_model_11(self, x,label_attention_map,is_training=True):         # x= [batch, n_step, w, h, c]
        """
       chen single
        """

        predict_label,cnn_out = self.single_CNNoutput_chen(x,is_training=is_training) #B,6,2
        self.predict_label = predict_label
        return predict_label,cnn_out
    ...
    def multi_CNNoutput_pretrainedCVPR_polar(self, x, label_attention_map,polar,is_training):
        ...
        for i in range(n_steps):
            ...
            polar_0=polar[:, i, :, :, :]
            if i == 0:
                y_0,_=net.get_label_attention_polar_inference(x_0, label_attention_map_0,polar_0,reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                y_0,_=net.get_label_attention_polar_inference(x_0, label_attention_map_0,polar_0,reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y,y_0], 1)

        return y

    def single_CNNoutput_pretrainedCVPR_polar(self, x, label_attention_map,polar,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps = x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            polar_0 = polar[:, i, :, :, :]
            if i == 0:
                _,y_0 = net.get_label_attention_polar_inference(x_0, label_attention_map_0, polar_0, reuse=False)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
            else:
                _,y_0 = net.get_label_attention_polar_inference(x_0, label_attention_map_0, polar_0, reuse=True)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y, y_0], 1)

        return y
    def single_CNNoutput_pretrainedCVPR(self, x, label_attention_map,is_training):
        net = CVPR_Network.Net()
        net.is_training = True
        n_steps = x.get_shape().as_list()[1]
        for i in range(n_steps):
            ...
            if i == 0:
                y_0,cnn_out0 = net.get_label_attention_inference_fc(x_0, label_attention_map_0,  reuse=False)
                cnn_out0 = tf.expand_dims(cnn_out0, 1)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
                cnn_out=cnn_out0
            else:
                y_0, cnn_out0 = net.get_label_attention_inference_fc(x_0, label_attention_map_0,  reuse=True)
                cnn_out0 = tf.expand_dims(cnn_out0, 1)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y, y_0], 1)
                cnn_out = tf.concat([cnn_out, cnn_out0], 1)

        return y,cnn_out
    def single_CNNoutput_chen(self, x,is_training):
        net = ChenNet()
        ...
        for i in range(n_steps):
            ...
            if i == 0:
                y_0, cnn_out0 = net.inference(x_0, reuse=False)
                cnn_out0 = tf.expand_dims(cnn_out0, 1)
                y_0 = tf.expand_dims(y_0, 1)
                y = y_0
                cnn_out = cnn_out0
            else:
                y_0, cnn_out0 = net.inference(x_0, reuse=True)
                cnn_out0 = tf.expand_dims(cnn_out0, 1)
                y_0 = tf.expand_dims(y_0, 1)
                y = tf.concat([y, y_0], 1)
                cnn_out = tf.concat([cnn_out, cnn_out0], 1)
    ...
    def _loss_per_batch(self, predict_label, labGT):  # label_predict_op=(batch, n_steps, n_outputs)  label=[batch, n_steps]
        ...
        ce=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=predict_label, labels=labGT) #[batch_size]
